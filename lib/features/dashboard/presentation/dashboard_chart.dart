...
class DashboardChart extends StatefulWidget {
  ...
  @override
  State<DashboardChart> createState() => DashboardChartState();
}
...
class DashboardChartState extends State<DashboardChart> {
  ...
  Widget build(BuildContext context) {
    final List<Map<String, String>> recentData =
        (maxItems != null && widget.data.length > maxItems!)
        ? widget.data.sublist(widget.data.length - maxItems!)
        : widget.data;
    final chartSpots = generateSpots(recentData);
    final lineColor = Colors.red.shade900;

    final double dateInterval = recentData.length <= 7
        ? 1.0
        : (recentData.length <= 30
              ? 5.0
              : (recentData.length / 6).ceilToDouble());
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [5, 7, 30, null].map((filterValue) {
            final isSelected = maxItems == filterValue;
            final label = filterValue == null ? 'All' : '$filterValue days';
                ...
                touchTooltipData: LineTouchTooltipData(
                  ...
                  getTooltipItems: (List<LineBarSpot> touchedSpots) {
                    return touchedSpots.map((spot) {
                      ...
                      String dateStr = recentData[index]["date"] ?? "Unknown";
                  ...
                  sideTitles: SideTitles(
                    ...
                    getTitlesWidget: (value, meta) {
                      ...
                      if (index >= 0 && index < recentData.length) {
                        String dateStr = recentData[index]["date"] ?? "";
